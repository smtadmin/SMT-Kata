...
public class WiggleSequence {
	...
	public int count(int[] sequence) {
		...
		// Use a linked list so we can easily peek at the elements
		LinkedList<Integer> wiggle = new LinkedList<>();
		
		// Assign booleans for up down and whether the loop is stick
		boolean isClear = true;
		boolean higher = true;
		int val = 0;
		
		// Loop the elments and see if they wiggle.  If no wiggle, skip over
		for (int i=0; i < sequence.length - 1; i++) {
			// Assign the current val and the next val.  If not a wiggle, val stays the same
			if(isClear) val = sequence[i];
			int nextVal = sequence[i + 1];
			
			// Check the wiggle and add tot he list if it is
			if ((higher && nextVal > val) || (! higher && nextVal < val)) {
				higher = !higher;
				isClear = true;
				
				// Check and see if the last element in the list matches the current value
				if(wiggle.isEmpty() || wiggle.peekLast() != val) wiggle.addLast(val);
				wiggle.addLast(nextVal);
			} else {
				isClear = false;
			}
		}
		
		// Number of wiggles equals the size of the list
		return wiggle.size();
	}
}
